        ...
        if (sender instanceof Player) {
            ...
            if (args.length >= 1) {
                if (getServer().getPlayer(args[0]) != null) {
                    if (player.hasPermission("mxt.gmc.others") || player.hasPermission("mxt.gm.others.*") || player.hasPermission("mxt.*")) {
            ...
            if (args.length == 0) {
                if (player.hasPermission("mxt.gmc") || player.hasPermission("mxt.gm.*") || player.hasPermission("mxt.*")) {
            ...
            if (args.length == 0) {
                ...
                if (getServer().getPlayer(args[0]) != null) {
                    Player target = Bukkit.getPlayer(args[0]);
                    target.setGameMode(GameMode.CREATIVE);
                    target.sendMessage(ChatColor.GREEN + "Your gamemode has set to " + ChatColor.RED + "CREATIVE" + ChatColor.GREEN + " by " + ChatColor.RED + "CONSOLE");
                    System.out.println("You have set " + target.getName() + " gamemode to " + "CREATIVE");
                } else {
